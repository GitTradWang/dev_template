...
class SCardView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    init {
        val a = context.obtainStyledAttributes(attrs, R.styleable.SCardView, defStyleAttr, R.style.CardView)
        ...
        if (a.hasValue(R.styleable.SCardView_cardBackgroundColor)) {
            ...
            backgroundColor = ColorStateList.valueOf(if (hsv[2] > 0.5f)
                resources.getColor(R.color.bl_card_view_light_background)
            else
                resources.getColor(R.color.bl_card_view_dark_background))
        }
        ...
        mContentPadding.left = a.getDimensionPixelSize(R.styleable.SCardView_contentPaddingLeft, defaultPadding)
        mContentPadding.top = a.getDimensionPixelSize(R.styleable.SCardView_contentPaddingTop, defaultPadding)
        mContentPadding.right = a.getDimensionPixelSize(R.styleable.SCardView_contentPaddingRight, defaultPadding)
        mContentPadding.bottom = a.getDimensionPixelSize(R.styleable.SCardView_contentPaddingBottom, defaultPadding)
        if (elevation > maxElevation) {
            maxElevation = elevation
        }
        val direction = a.getInt(R.styleable.SCardView_cardLightDirection, DIRECTION_TOP)
        val directionRadio = a.getFloat(R.styleable.SCardView_cardLightDirectionRatio, 0f).let {
            when {
                it > 0.6f -> 0.6f
                it < 0.0 -> 0f
                else -> it
        ...
        IMPL.initialize(mCardViewDelegate, context, backgroundColor, radius, elevation, maxElevation, direction, directionRadio, cardCornerVisibility, shadowStartColor, shadowEndColor)
    }
