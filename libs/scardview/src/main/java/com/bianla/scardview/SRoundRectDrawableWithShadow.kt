 ...
 *  2018/9/11.
 */
class SRoundRectDrawableWithShadow(
        cardViewDelegate: SCardViewDelegate,
        resources: Resources,
        ...
        maxShadowSize: Float,
        direction: Int,
        directionRadio: Float,
        cornerVisibility: Int,
        startColor: Int,
    ...
    * */
    ...
    private var mLightDirectionRadio = 0.5f
    ...
    init {
        mShadowStartColor = if (startColor == -1) resources.getColor(R.color.bl_card_view_shadow_start_color) else startColor
        mShadowEndColor = if (endColor == -1) resources.getColor(R.color.bl_card_view_shadow_end_color) else endColor
        mInsetShadow = resources.getDimensionPixelSize(R.dimen.card_view_compat_inset_shadow)
        ...
        mLightDirectionRadio = directionRadio
    ...
    private fun calculateShadowDirection(): Pair<Pair<Float, Float>, Pair<Float, Float>> {
        ...
        val moveDistance = mRawShadowSize * mLightDirectionRadio
    ...
    private fun calculateCornerVisibility(): RectF {
        return when (mCornerVisibility) {
            NO_LEFT_CORNER -> RectF(0f, mCornerRadius, mCornerRadius, 0f)
            NO_RIGHT_CORNER -> RectF(mCornerRadius, 0f, 0f, mCornerRadius)
            NO_TOP_CORNER -> RectF(0f, 0f, mCornerRadius, mCornerRadius)
            NO_BOTTOM_CORNER -> RectF(mCornerRadius, mCornerRadius, 0f, 0f)
            NO_LT_RB_CORNER -> RectF(0f, mCornerRadius, 0f, mCornerRadius)
            NO_RT_LB_CORNER -> RectF(mCornerRadius, 0f, mCornerRadius, 0f)
    ...
    private fun drawRTCorner(canvas: Canvas, cornerRadius: Float) {
        ...
        if (drawVerticalEdges) {
            if (mCornerVisibility == NO_TOP_CORNER || mCornerVisibility == NO_RT_LB_CORNER) {
                right -= mCornerRadius
            }
            if (mCornerVisibility == NO_BOTTOM_CORNER || mCornerVisibility == NO_LT_RB_CORNER) {
                right += mCornerRadius
            }
    ...
    private fun drawRBCorner(canvas: Canvas, cornerRadius: Float) {
        ...
        if (drawHorizontalEdges) {
            ...
            if (mCornerVisibility == NO_LEFT_CORNER || mCornerVisibility == NO_RT_LB_CORNER)
                right += mCornerRadius
            if (mCornerVisibility == NO_RIGHT_CORNER || mCornerVisibility == NO_LT_RB_CORNER)
                right -= mCornerRadius
            canvas.drawRect(0f, edgeShadowTop, right, -cornerRadius, mEdgeShadowPaint)
        }
    }
    ...
    private fun drawLBCorner(canvas: Canvas, cornerRadius: Float) {
        ...
        canvas.drawPath(mCornerShadowPath!!, mCornerShadowPaint)
        if (drawVerticalEdges) {
            if (mCornerVisibility == NO_TOP_CORNER || mCornerVisibility == NO_LT_RB_CORNER) {
                right += mCornerRadius
            }
            if (mCornerVisibility == NO_BOTTOM_CORNER || mCornerVisibility == NO_RT_LB_CORNER) {
                right -= mCornerRadius
            }
            canvas.drawRect(0f, edgeShadowTop, right, -cornerRadius, mEdgeShadowPaint)
        }
    ...
    private fun drawLTCorner(canvas: Canvas, cornerRadius: Float) {
        ...
        if (drawHorizontalEdges) {
            var right = mCardBounds.width() - 2 * inset
            if (mCornerVisibility == NO_RIGHT_CORNER || mCornerVisibility == NO_RT_LB_CORNER)
                right += mCornerRadius
            if (mCornerVisibility == NO_LEFT_CORNER || mCornerVisibility == NO_LT_RB_CORNER)
                right -= mCornerRadius
            canvas.drawRect(0f, edgeShadowTop, right, -cornerRadius, mEdgeShadowPaint)
        }
    }
    ...
    private fun buildShadowCorners(cornerRadius: Float) {
        ...
        mCornerShadowPaint.shader = RadialGradient(0f, 0f, cornerRadius + mShadowSize,
                intArrayOf(mShadowStartColor, mShadowStartColor, mShadowEndColor), floatArrayOf(0f, startRatio, 1f), Shader.TileMode.CLAMP)

        // we offset the content shadowSize/2 pixels up to make it more realistic.
        // this is why edge shadow shader has some extra space
        // When drawing bottom edge shadow, we use that extra space.
        mEdgeShadowPaint.shader = LinearGradient(0f, -cornerRadius + mShadowSize, 0f, -cornerRadius - mShadowSize,
                intArrayOf(mShadowStartColor, mShadowStartColor, mShadowEndColor), floatArrayOf(0f, .5f, 1f), Shader.TileMode.CLAMP)
        mEdgeShadowPaint.isAntiAlias = false
    }
