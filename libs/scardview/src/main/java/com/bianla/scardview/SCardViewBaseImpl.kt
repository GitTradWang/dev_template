...
open class SCardViewBaseImpl : SCardViewImpl {
        ...
        SRoundRectDrawableWithShadow.sRoundRectHelper = object : SRoundRectDrawableWithShadow.RoundRectHelper {
            override fun drawRoundRect(canvas: Canvas, bounds: RectF, cornerRadius: Float, cornerVisibility: Int, paint: Paint) {
                ...
                if (cornerRadius >= 1f) {
                    ...
                    if (cornerVisibility == NO_LEFT_CORNER || cornerVisibility == NO_TOP_CORNER || cornerVisibility == NO_LT_RB_CORNER)
                    ...
                    canvas.rotate(90f)
                    if (cornerVisibility == NO_RIGHT_CORNER || cornerVisibility == NO_TOP_CORNER || cornerVisibility == NO_RT_LB_CORNER)
                        canvas.drawRect(-roundedCornerRadius, -roundedCornerRadius, 0f, 0f, paint)
                    else
                        canvas.drawArc(mCornerRect, 180f, 90f, true, paint)
                    canvas.translate(innerHeight, 0f)
                    canvas.rotate(90f)
                    if (cornerVisibility == NO_RIGHT_CORNER || cornerVisibility == NO_BOTTOM_CORNER || cornerVisibility == NO_LT_RB_CORNER)
                        canvas.drawRect(-roundedCornerRadius, -roundedCornerRadius, 0f, 0f, paint)
                    else
                        canvas.drawArc(mCornerRect, 180f, 90f, true, paint)
                    canvas.translate(innerWidth, 0f)
                    canvas.rotate(90f)
                    if (cornerVisibility == NO_LEFT_CORNER || cornerVisibility == NO_BOTTOM_CORNER || cornerVisibility == NO_RT_LB_CORNER)
                        canvas.drawRect(-roundedCornerRadius, -roundedCornerRadius, 0f, 0f, paint)
                    else
                        canvas.drawArc(mCornerRect, 180f, 90f, true, paint)
    ...
    private fun createBackground(cardViewDelegate: SCardViewDelegate,
                                 context: Context,
                                 backgroundColor: ColorStateList,
                                 radius: Float,
                                 elevation: Float,
        ...
        return SRoundRectDrawableWithShadow(cardViewDelegate, context.resources, backgroundColor, radius, elevation, maxElevation, direction,directionRadio, cornerVisibility, startColor, endColor)
    }
